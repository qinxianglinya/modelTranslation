...
{
	//    cout<<"nms getOutputDimensions"<<endl;
	//    cout<<"nbInputDims"<<nbInputDims<<endl;
	...
	int batchSize, const void* const* inputs, void** outputs, void* workspace, cudaStream_t stream)
{
	...
	const void* const priorData = inputs[param.inputOrder[2]];
	   /* float* resultPrior = new float[409200];
	     cudaMemcpy(resultPrior, priorData,409200 * sizeof(float), cudaMemcpyDeviceToHost);
	     for(int i=0; i<8; i++)
	     {
	         cout<<"priorData:"<<i<<"  "<<resultPrior[i]<<endl;
	     }
	     float* resultLoc = new float[153600];
	     cudaMemcpy(resultLoc, locData,153600 * sizeof(float), cudaMemcpyDeviceToHost);
	     for(int i=0; i<12; i++)
	     {
	         if(i%4==0)
	         {
	         ...
	         cout<<"offset:"<<resultLoc[i]<<endl;
	     }
	     float* resultConf = new float[230401];
	     cudaMemcpy(resultConf, confData,230400 * sizeof(float), cudaMemcpyDeviceToHost);
	     for(int i=0; i<18; i++)
	     {
	         if(i%6==0)
	         {
	         ...
	         cout<<"confidence:"<<resultConf[i]<<endl;
	     }*/
	...
	//std::cout<<"numclass:"<<param.numClasses<<std::endl;
